...
class UltimateTradingSystem:
    ...
    def execute_trade(self, signal: TradingSignal) -> Optional[Position]:
        ...
        # 计算移动止损
        atr = signal.indicators.get('atr', signal.entry_price * 0.02)
...
def main():
    ...
    print(f"🌐 当前市场状态: {market_condition.value}")
    ...
    if signal:
        ...
        print(f"🌐 市场状态: {signal.market_condition.value}")
        ...
        if position:
            ...
            print(f"📊 移动止损: ${position.trailing_stop:.2f}")
        else:
            print("❌ 交易未执行（风险管理限制）")
    else:
        print("❌ 未生成有效交易信号")
    ...
    if indicators:
        ...
        print(f"   成交量比: {indicators.get('volume_ratio', 0):.2f}")
        print(f"   动量: {indicators.get('momentum', 0):.2f}")
        print(f"   终极振荡器: {indicators.get('ultosc', 0):.2f}")
    ...
    print(f"\n🧠 机器学习分析:")
    print(f"   预测方向: {'上涨' if ml_analysis['classification'] > 0 else '下跌' if ml_analysis['classification'] < 0 else '持平'}")
    print(f"   预测置信度: {ml_analysis['confidence']:.2f}")
    print(f"   预期收益率: {ml_analysis['regression_pred']*100:.2f}%")
